...
const Home = () => {
    ...
    const onFormSubmit = async (event: { preventDefault: () => void; }) => {
        try {
            ...
            setLoading(true);
            const response = await axios.get('https://fcp2w0ka75.execute-api.ap-northeast-1.amazonaws.com/production', {
        ...
        <div className="ui container" id="container">
            <div className="Search__Form">
                <form className="ui form segment" onSubmit={onFormSubmit}>
                    ...
                    <div className="field">
                        <label><i className="map pin icon"></i>移動時間計算の出発拠点（自宅から近い拠点をお選びください）</label>
                        <select
                            className="ui dropdown"
                            name="dropdown"
                            value={departure}
                    ...
                    <div className="Search__Button">
                        <button type="submit" className="Search__Button__Design">
                            <i className="search icon"></i>ゴルフ場を計算する
                        </button>
                    </div>
                </form>
